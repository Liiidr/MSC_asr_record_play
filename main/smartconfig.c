...
void sc_callback(smartconfig_status_t status, void *pdata)
{
    ...
    switch (status) {
        ...
        case SC_STATUS_LINK:
            ...
            printf("PASSWORD:%s", wifi_config->sta.password);
...
void smartconfig_example_task(void * parm)
{
    ...
    while (1) {
        uxBits = xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT | ESPTOUCH_DONE_BIT, true, false, 10000/portTICK_PERIOD_MS); 
